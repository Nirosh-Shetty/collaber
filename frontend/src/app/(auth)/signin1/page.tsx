...
import { EyeIcon, EyeOffIcon, UserIcon, ArrowLeftIcon } from "lucide-react"
import Link from "next/link"
import axios from "axios"

// Set Password Form Component for OAuth users
function SetPasswordForm({ email, onSuccess, onCancel }: { email: string; onSuccess: () => void; onCancel: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isPasswordValid) newErrors.password = "Password must be 8+ chars with letters and numbers"
    ...
    try {
      await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/auth/set-password-oauth`, {
        email,
      ...
      setErrors({ submit: error.response?.data?.message || "Failed to set password" })
    } finally {
  ...
  return (
    <div className="bg-purple-500/10 border border-purple-500/30 rounded-lg p-4 space-y-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-white font-semibold text-sm">Set Your Password</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="space-y-1">
          ...
          <div className="relative">
            <Input
              ...
              className="bg-white/5 border-white/20 text-white placeholder-gray-500 text-sm pr-8 h-8"
            />
            <button
              type="button"
          ...
          {formData.password && (
            <div className="space-y-0.5">
              {[
                ...
                { check: passwordStrength.hasLetter, label: "Contains letters" },
                { check: passwordStrength.hasNumber, label: "Contains numbers" },
              ].map((item) => (
                <div key={item.label} className="flex items-center text-xs">
                  <div className={`w-1 h-1 rounded-full mr-1.5 ${item.check ? "bg-green-500" : "bg-gray-600"}`}></div>
                  <span className={item.check ? "text-green-400" : "text-gray-500"}>{item.label}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="space-y-1">
          ...
          <Input
            ...
            className="bg-white/5 border-white/20 text-white placeholder-gray-500 text-sm h-8"
          />
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.response?.data?.message) {
        ...
        setCanSetPassword(error.response?.data?.canSetPassword || false)
        ...
        <Card className="bg-white/5 backdrop-blur-sm border-white/10">
          <CardContent className="p-8">
            ...
            <form className="space-y-6" onSubmit={handleSignIn}>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Link href="/forgot-password1" className="text-xs text-purple-400 hover:text-purple-300">
                    Forgot password?
                  </Link>
                </div>
                <div className="relative">
              ...
              {error && (
                <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
                  ...
                  {canSetPassword && !showSetPasswordForm && (
                    <div className="text-xs text-red-300 space-y-2">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <Link href="/forgot-password1" className="w-full">
                          <Button
                            type="button"
                            variant="outline"
                            size="sm"
                            className="bg-red-500/20 border-red-500/50 text-red-200 hover:bg-red-500/30 h-8 text-xs w-full"
                          >
                      ...
                      {linkedAccounts.length > 0 && (
                        <>
                          <p className="mt-3 mb-2">Or sign in with:</p>
                          <div className="flex gap-2">
                            {linkedAccounts.includes("google") && (
                              <Button
                                ...
                                variant="outline"
                                size="sm"
                                className="bg-red-500/20 border-red-500/50 text-red-200 hover:bg-red-500/30 h-8 flex-1"
                  ...
                  {linkedAccounts.length > 0 && !canSetPassword && (
              ...
              {showSetPasswordForm && canSetPassword && (
                <SetPasswordForm email={userEmail} onSuccess={() => {
                  setShowSetPasswordForm(false)
                  setError("")
                  setCanSetPassword(false)
                  alert("Password set successfully! You can now sign in with your email and password.")
                }} onCancel={() => setShowSetPasswordForm(false)} />
              )}
            ...
            <p className="text-center text-sm text-gray-400">
              ...
              <Link href="/signup1/welcome" className="text-purple-400 hover:text-purple-300">
                Sign up
              </Link>
            </p>
          </CardContent>
