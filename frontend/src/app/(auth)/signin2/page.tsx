  ...
  const handleSignIn = async (data: z.infer<typeof signInSchema>) => {
    ...
    try {
      ...
      if (error.response) {
        ...
        // Capture linkedAccounts for OAuth-only errors
        if (error.response?.data?.linkedAccounts) {
    ...
    <div className="container max-w-md mx-auto py-10 px-4">
      <div className="space-y-6">
        ...
        <form className="space-y-4" onSubmit={handleSubmit(handleSignIn)}>
          ...
          {/* Show options if account doesn't have password */}
          {linkedAccounts.length > 0 && (
            <div className=\"bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 rounded-lg p-3 space-y-2\">
              <p className=\"text-xs text-red-700 dark:text-red-300 font-semibold\">{errors?.identifier?.message}</p>
              <p className=\"text-xs text-red-600 dark:text-red-400\">You have 2 options:</p>
              <div className=\"space-y-2\">
                {linkedAccounts.includes(\"google\") && (
                  ...
                  </Button>\n                )}\n                {linkedAccounts.includes(\"facebook\") && (\n                  <Button\n                    type=\"button\"\n                    variant=\"outline\"\n                    className=\"w-full\"\n                    onClick={() =>\n                      window.location.href = `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/auth/facebook`\n                    }\n                  >\n                    <svg\n                      xmlns=\"http://www.w3.org/2000/svg\"\n                      viewBox=\"0 0 24 24\"\n                      className=\"h-4 w-4 mr-2 text-blue-600 fill-current\"\n                    >\n                      <path d=\"M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z\" />\n                    </svg>\n                    Continue with Facebook\n                  </Button>\n                )}\n                <Link href=\"/forgot-password2\" className=\"block\">\n                  <Button\n                    type=\"button\"\n                    variant=\"outline\"\n                    className=\"w-full\"\n                  >\n                    Reset Password via Email\n                  </Button>\n                </Link>\n              </div>\n            </div>\n          )}\n\n          <Button type=\"submit\" className=\"w-full\" disabled={isLoading}>\n            {isLoading ? \"Signing in...\" : \"Sign In\"}\n          </Button>
            {isLoading ? "Signing in..." : "Sign In"}
          </Button>
        </form>
